...
# Roles that may administer an organization (same set as users/departments).
...
def _build_schemas(model):
    ...
    for col in model.__table__.columns:
        ...
        if col.nullable:
            ...
            create_fields[col.name] = (Optional[py], None) if has_default else (py, ...)
...
def crud_router(
    ...
    *,
    ...
    def list_items(
        ...
        if order_by_desc and hasattr(model, order_by_desc):
            query = query.order_by(getattr(model, order_by_desc).desc())
        return query.all()
